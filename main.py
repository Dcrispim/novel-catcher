...
def get_all_url_link(soup_title: BeautifulSoup):
    ...
    first_title = [link['href']
                   for link in soup_title.select(CHAPTER_SELECTOR)]
    return first_title


def get_all_volumes(soup_title: BeautifulSoup):

    first_title = [(link.text.strip(), link['id'])
                   for link in soup_title.select(VOLUME_SELECTOR)]
    return first_title
...
def make_chapter_text_from_link(url):
    ...
    chapter_text: BeautifulSoup = soup_chapter.select_one(
        CONTENT_CHAPTER_SELECTOR)
...
def make_chapter_text_from_volume(name):
    ...
    for volume in volume_list:
        ...
        chapter_list = get_all_url_link_by_volume(
            soup_title, volume[0], volume[1])
        for chapter_name in chapter_list.keys():
            chapter_text = make_chapter_text_from_link(
                chapter_list[chapter_name])
            out_text += f' \n\n {chapter_text}'
        print('Crerating ', volume[0])
        make_pdf_from_html_text(out_text, os.path.join(
            BASEDIR, 'dist', f'{name.replace("-"," ").title()}-{volume[0]}.pdf'), './styles/novel.css')
        print('End of ', volume[0])
        with open(os.path.join(BASEDIR, 'temp', f'{name}-{volume[0]}.html'.replace(' ', '_')), 'a') as File:
            File.write(out_text)
